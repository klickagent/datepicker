...
export class DateTimePickerComponent implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    public ngOnInit() {
        ...
        // todo (improve): add scroll only to element (parent)
    ...
    */
    ...
    private show( event: any ): void {
        ...
        this.elementDimensions = element.offsetParent ? {
    ...
    private alignDialog(): void {
        ...
        let inputEl = this.textInputElm.nativeElement;
        let inputHeight = inputEl.offsetHeight;
        
        
        let targetHeight = target.offsetHeight + target.offsetTop;
        let targetWidth = target.offsetWidth;
        let targetOffset = target.getBoundingClientRect();
        let viewport = this.getViewport();
        let top, left;

        if ((targetOffset.top + inputHeight + this.elementDimensions.height) > viewport.height) {
            top = 0;
        } else {
            top = targetOffset.top + inputHeight;
        }

        if ((targetOffset.left + this.elementDimensions.width) < viewport.width) {
            left = targetOffset.left;
        } else {
            left = 0;
        }

        //todo on scoll if fixed => close item
        //todo align in center if out of window-frame (landscape ios)
